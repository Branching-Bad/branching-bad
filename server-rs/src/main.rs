...
use crate::{
    ...
    provider::cloudwatch::{aws_client::AwsClient, investigator},
...
async fn provider_list_bindings(
    ...
    // Include stored resources for each bound account (no remote API call)
    ...
    Ok(Json(json!({ "bindings": filtered, "resources": stored_resources })))
}
...
async fn cw_investigate(
    ...
    // Get AWS credentials from provider account
    let account = state
        .db
        .get_provider_account(&payload.account_id)
        .map_err(ApiError::internal)?
    ...
    // Get agent command for this repo
    let agent_command = resolve_agent_command(&state, &payload.repo_id)
    ...
    // Spawn background task for Phase 1
    tokio::task::spawn_blocking(move || {
...
async fn cw_analyze(
    ...
    let agent_command = resolve_agent_command(&state, &inv.repo_id)
        .ok_or_else(|| ApiError::bad_request("Select an AI profile for this repo first."))?;
...
async fn cw_run_saved_query(
    ...
    // Run saved query directly against CW (skip agent)
    tokio::spawn(async move {
        let access_key = config["access_key_id"].as_str().unwrap_or_default();
        let secret_key = config["secret_access_key"].as_str().unwrap_or_default();
        let region = config["region"].as_str().unwrap_or_default();
