...
export function EsInvestigateModal(props: Props) {
  const {
    open,
    onClose,
    investigationId,
  ...
  const fetchInvestigation = useCallback(async () => {
    ...
    try {
      const payload = await api<{ investigation: InvestigationData }>(
        `/api/elasticsearch/investigations/${investigationId}`
      );
  ...
  async function handleAnalyze() {
    onBusyChange(true);
    try {
      await api(`/api/elasticsearch/investigations/${investigationId}/analyze`, { method: "POST" });
      void fetchInvestigation();
      startPolling();
    } catch (e) {
  ...
  async function handleRegenerate() {
    onBusyChange(true);
    try {
      await api(`/api/elasticsearch/investigations/${investigationId}/regenerate`, { method: "POST" });
      void fetchInvestigation();
      startPolling();
    } catch (e) {
      onError((e as Error).message);
  ...
  async function handleSaveQuery() {
    ...
    try {
      const queryStr = data.query_phase1 ?? JSON.stringify(result.phase1_query ?? {});
      await api("/api/elasticsearch/saved-queries", {
        method: "POST",
        body: JSON.stringify({
          repoId: selectedRepoId,
          indexPattern: data.index_pattern,
          label: saveLabel.trim(),
          question: data.question,
          queryTemplate: queryStr,
          keywords: "",
        }),
      });
      onInfo("Query saved.");
  ...
  async function handleCreateTask() {
    ...
    try {
      const res = await api<{ task: { id: string; title: string } }>(
        `/api/elasticsearch/investigations/${investigationId}/create-task`,
        { method: "POST" }
      );
  ...
  function copyQuery() {
    const q = data?.query_phase1 ?? JSON.stringify(result.phase1_query ?? {}, null, 2);
  ...
  function formatQuery(): string {
    ...
    if (result.phase1_query) {
      return JSON.stringify(result.phase1_query, null, 2);
    }
    return "(no query yet)";
  }
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 text-xs">
          ...
          {data && ["running", "analyzing"].includes(data.status) && (
            <div className="flex items-center gap-2 text-text-muted py-8 justify-center">
              ...
              <span>{data.status === "running" ? "Agent is generating ES query..." : "Agent is analyzing logs..."}</span>
            </div>
          )}
